---
title: "Training Multi-Agentic Systems for Complex Task Planning with GRPO Algorithm | by Fareed Khan | in Level Up Coding - Freedium"
source: "https://freedium-mirror.cfd/https://levelup.gitconnected.com/training-multi-agentic-systems-for-complex-task-planning-with-grpo-algorithm-f698e831d730?gi=06deec38a814&source=email-5b07b876ba3e-1770752726693-digest.reader-5517fd7b58a6-f698e831d730----0-102------------------f19b634e_b5f8_4ff4_9be7_77f7ac6a8812-1"
author:
published:
created: 2026-04-06
description: "Rollouts, Advantages, Trajectories, Rewards and more"
tags:
  - "clippings"
---
[< Go to the original](https://levelup.gitconnected.com/training-multi-agentic-systems-for-complex-task-planning-with-grpo-algorithm-f698e831d730#bypass)

![Preview image](https://miro.medium.com/v2/resize:fit:700/1*nU5tAGaU3z4vO3apzIYblg.png)
...
[![Fareed Khan](https://miro.medium.com/v2/resize:fill:88:88/1*feiUXOR8sid6IPHSHufA-g.jpeg)](https://medium.com/@fareedkhandev "I write on AI, https://www.linkedin.com/in/fareed-khan-dev/")

[Fareed Khan](https://medium.com/@fareedkhandev "I write on AI, https://www.linkedin.com/in/fareed-khan-dev/")[Level Up Coding](https://medium.com/gitconnected "Coding tutorials and news.")a11y-light ~59 min read · February 9, 2026 (Updated: February 9, 2026) · Free: No

Read this story for free: [link](https://medium.com/@fareedkhandev/f698e831d730?source=friends_link&sk=d5e9bdd83018c7e7b1358b45550953e8)

Agentic systems for long-horizon tasks require **planning**, correct **tool use**, and **step-by-step execution**. Most modern agentic systems rely on inference, start fresh with each component each time, and lack prior training. This increases the chances of **wrong planning** or incorrect tool calls at any step in long-horizon tasks. The **GRPO algorithm**, a modern RL approach, continuously **trains agents to plan** and execute correctly for extended tasks. A typical GRPO-based agentic training system looks like this…
...
## [GitHub - FareedKhan-dev/multi-agent-training-grpo: A multi-agent system trained with GRPO for…](https://github.com/FareedKhan-dev/multi-agent-training-grpo)

### A multi-agent system trained with GRPO for reliable long-horizon task planning and execution. …

github.com

The codebase is organized as follows (Theory + Step by step Workflow):
...
└── GRPO_Training_Agentic/
    ...
    └── utils.py                       # Utils of agents for training
```

### Table of Content

- [Role of GRPO Algorithm in Agent System](#2cba "Role of GRPO Algorithm in Agent System")
- [Agentic Data Preprocessing](#ef50 "Agentic Data Preprocessing")
- [Building Multi-Agentic Architecture](#343d "Building Multi-Agentic Architecture") ∘ [Defining the Agent Thoughts](#5770 "Defining the Agent Thoughts") ∘ [Creating The Toolset](#1c92 "Creating The Toolset") ∘ [Solver, Planner, Executor, and Verifier](#da62 "Solver, Planner, Executor, and Verifier") ∘ [Orchestrating the Agentic Loop](#a458 "Orchestrating the Agentic Loop")
- [Analyzing Query Hallucinations](#a7eb "Analyzing Query Hallucinations")
- [Agentic GRPO Algorithm Implementation](#9be9 "Agentic GRPO Algorithm Implementation") ∘ [Initializing the Policy Model (QLoRA & PEFT)](#ef9c "Initializing the Policy Model (QLoRA & PEFT)") ∘ [Agentic System Wrapper for Rollouts](#2821 "Agentic System Wrapper for Rollouts") ∘ [Generating Trajectories & Computing Log Probs](#e7ef "Generating Trajectories & Computing Log Probs") ∘ [Reward Modeling with GPT-4o](#3169 "Reward Modeling with GPT-4o") ∘ [Creating Advantages and PPO Loss](#f971 "Creating Advantages and PPO Loss") ∘ [Running GRPO Training Loop](#3af1 "Running GRPO Training Loop")
- [Running the Optimized Planning Agent](#0cb4 "Running the Optimized Planning Agent")
...
![None](https://miro.medium.com/v2/resize:fit:700/1*LR0z01V9CiGviaLZu4CSSg.png)

GRPO in Agentic System (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
2. **The Planner Policy generates a group of distinct trajectories** (e.g., 4 attempts) for this single query. Because the model uses a non-zero temperature, it explores different strategies: one trajectory might correctly use Python to calculate **122122**, another might guess the number incorrectly, and a third might hallucinate a historical event without searching.
...
![None](https://miro.medium.com/v2/resize:fit:700/1*UgjcWaUWfnO9V3Ahu__cvg.png)

Data Preprocessing (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
![None](https://miro.medium.com/v2/resize:fit:700/1*M9nHSlNVxpPLjxezZxRJwg.png)

Multi-Agent System (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
![None](https://miro.medium.com/v2/resize:fit:700/1*tfzZJKBGXMpcqGimf8zwVA.png)

Agent thoughts (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
![None](https://miro.medium.com/v2/resize:fit:700/1*Eur15ZyWZTy1qGNGIc0Jbg.png)

Solver, planner, Executor (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
Response Format:
5.  **Justification:** Explain your choice of tool and sub-goal.
6.  **Context:** Provide all necessary information for the tool.
7.  **Sub-Goal:** State the specific objective for the tool.
8.  **Tool Name:** State the exact name of the selected tool."""
...
![None](https://miro.medium.com/v2/resize:fit:700/1*qOqaSIbRuDAiGAopyTm6nA.png)

Agent Loop (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
Output Structure:
4.  **Process Summary:** A clear, step-by-step breakdown of how the query was addressed.
5.  **Answer:** A direct and concise final answer to the query."""
...
![None](https://miro.medium.com/v2/resize:fit:700/1*bk2uTPyXc-D74Y_dZKfPdA.png)

GRPO Training (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
![None](https://miro.medium.com/v2/resize:fit:700/1*mGzgPhYBnNbySPc5tqnm1w.png)

Peft Config (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
![None](https://miro.medium.com/v2/resize:fit:700/1*lvtL1fI4uyo3IXCfuthxIA.png)

Agentic Wrapper for GRPO (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))

1. **Tool Management:** Loads and provides access to the specialized tools.
2. **State Generation:** Formulates the prompt (State St *St*) for the Planner based on the query and memory.
...
This method takes the current context (query and memory) and formats it into a cohesive prompt. This prompt represents the current **State (**St *St***)** observed by the Policy model.
...
![None](https://miro.medium.com/v2/resize:fit:700/1*tBNpo8WDZCnLoF84O14e7g.png)

Trajectories (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))

This log probability (log⁡π(a∣s)log *π* (*a* ∣ *s*)) is the foundation of the PPO importance ratio.
...
The policy only generates a *plan* (the action At *At*). The environment must carry out that plan (Execution) and determine if the agent should continue (Verification). This task is delegated to the powerful, fixed LLM to ensure reliable tool usage and reflection, decoupling it from the trainable Policy model.
...
![None](https://miro.medium.com/v2/resize:fit:700/1*CKFw2KsOSfRbyIkqq489uQ.png)

Reward Modeling (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))
...
![None](https://miro.medium.com/v2/resize:fit:700/1*Q1_GCFRncfbLOxLymyMLwA.png)

Advantages and PPO (Created by [Fareed Khan](https://medium.com/u/b856005e5ecd))

1. **Clipped Surrogate Loss:** Ensures policy updates move in the direction of higher reward while remaining close to the reference policy (clipping parameter ϵ *ϵ*).
2. **KL Divergence Penalty:** A regularizer (KL\_coefKL\_coef) that prevents the policy from diverging too far from the reference model, ensuring training stability.
```python
...
1. **Group Rollouts:** For each query in the batch, N *N* trajectories are generated.
2. **Advantage Calculation:** The N *N* rewards are normalized against their group mean and standard deviation to calculate the **Advantages** (the GRPO signal).
...
![None](https://miro.medium.com/v2/resize:fit:700/1*sCD8pKeNw1FQGdyUQVYA6w.png)
...
Enabled tools:
['Generalist_Solution_Generator_Tool',
 'Python_Code_Generamarptor_Tool',
 'Ground_Google_Search_Tool',
 'Wikipedia_RAG_Search_Tool']
